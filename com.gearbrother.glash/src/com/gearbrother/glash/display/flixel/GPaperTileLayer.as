...
class TileBmp extends GBitmap {
	...
	public function TileBmp(x:int, y:int, zoom:int, tile:IGPaperTile, overviewUrl:GFile, overviewScaling:int) {
		...
		_overviewScaling = overviewScaling;
		load = GMain.instance.pool.getInstance(_tile.getTile(x, y, zoom), GLoadOper.queue) as GFile;
		overviewload = GMain.instance.pool.getInstance(_overviewFile, GLoadOper.queue) as GFile;
