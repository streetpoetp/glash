package com.gearbrother.glash.display.layout.impl {
	...
	public class FlowLayout extends EmptyLayout {
		...
		override public function preferredLayoutSize(target:GContainer):GDimension {
			...
			var nmembers:int = target.numChildren;

			var counted:int = 0;
			for (var i:int = 0; i < nmembers; i++) {
				var m:GNoScale = target.getChildAt(i) as GNoScale;
		...
		override public function layoutContainer(target:GContainer, boxsGrid:GBoxsGrid2):void {
			var td:GDimension = new GDimension(target.originalWidth, target.originalHeight);
