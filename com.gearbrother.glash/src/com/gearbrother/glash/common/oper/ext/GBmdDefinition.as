package com.gearbrother.glash.common.oper.ext {
	...
	public class GBmdDefinition extends GOper implements IGObject {
		private var definition:GDefinition;
		...
		private var _toBmdOper:GDisplayToBmdOper;
		...
		override public function execute():void {
			...
			definition = GMain.instance.pool.getInstance(definition, GDefinition.queue) as GDefinition;
		...
		private function _handleDefinitionEvent(event:Event = null):void {
			if (definition.resultType == GOper.RESULT_TYPE_SUCCESS) {
				...
				if (result is BitmapData) {
					...
					_toBmdOper.commit(GDisplayToBmdOper.queue);
				}
			} else {
				notifyFault();
			}
		...
		private function _handleBmdEvent(event:Event):void {
		...
		override public function dispose():void {
			GMain.instance.pool.returnInstance(definition);
			_toBmdOper.dispose();
