package com.gearbrother.glash.display.control {
	...
	public class GRange extends GNoScale {
		...
		public function GRange(skin:DisplayObject = null) {
			...
			if (skin && skin.hasOwnProperty("minButton")) {
				minButton = new GButtonLite(skin["minButton"]);
				minButton.autoRepeat = true;
				minButton.addEventListener(MouseEvent.CLICK, onButtonClick);
			}
			if (skin && skin.hasOwnProperty("maxButton")) {
				maxButton = new GButtonLite(skin["maxButton"]);
				maxButton.autoRepeat = true;
				maxButton.addEventListener(MouseEvent.CLICK, onButtonClick);
			}
		}
